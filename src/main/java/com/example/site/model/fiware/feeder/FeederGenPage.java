...
import com.example.site.model.fiware.feeder.Feeder;
 ...
 * Generated: true
 **/
public class FeederGenPage extends FeederGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListFeeder_(Wrap<SearchList<Feeder>> w) {
  }
  ...
  protected void _varsQ(JsonObject vars) {
    Feeder.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(Feeder.displayNameFeeder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(Feeder.classSimpleNameFeeder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListFeeder_.getRequest().getQuery()).filter(fq -> fq.startsWith(Feeder.varIndexedFeeder(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    for(String var : Feeder.varsFqForClass()) {
      String varIndexed = Feeder.varIndexedFeeder(var);
      String varStored = Feeder.varStoredFeeder(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(Feeder.displayNameFeeder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(Feeder.classSimpleNameFeeder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListFeeder_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(Feeder.varIndexedFeeder(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        json.put("val", v);
        varsFqCount++;
      }
  ...
  protected void _varsRange(JsonObject vars) {
    Feeder.varsRangeForClass().forEach(var -> {
      String varIndexed = Feeder.varIndexedFeeder(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(Feeder.displayNameFeeder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(Feeder.classSimpleNameFeeder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListFeeder_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(Feeder.varIndexedFeeder(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long rows1 = Optional.ofNullable(searchListFeeder_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListFeeder_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListFeeder_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = Feeder.searchVarFeeder(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", Feeder.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListFeeder_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = Feeder.searchVarFeeder(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", Feeder.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListFeeder_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListFeeder_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListFeeder_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListFeeder_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return Feeder.searchVarFeeder(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListFeeder_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListFeeder_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListFeeder_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListFeeder_.getFacetPivotMinCount()).orElse(0));
  }

  @Override
  protected void _DEFAULT_MAP_LOCATION(Wrap<JsonObject> w) {
    Point point = Feeder.staticSetLocation(siteRequest_, Optional.ofNullable(siteRequest_.getRequestVars().get(VAR_DEFAULT_MAP_LOCATION)).orElse(siteRequest_.getConfig().getString(ConfigKeys.DEFAULT_MAP_LOCATION)));
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListFeeder_.getDefaultSort()) {
      Optional.ofNullable(searchListFeeder_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = Feeder.searchVarFeeder(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListFeeder_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = Feeder.searchVarFeeder(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListFeeder_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListFeeder_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listFeeder(JsonArray l) {
    Optional.ofNullable(searchListFeeder_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListFeeder_ == null ? 0 : searchListFeeder_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<Feeder> w) {
  ...
  protected void _pageUri(Wrap<String> c) {
    c.o("/en-us/search/feeder");
  }

  @Override
  protected void _apiUri(Wrap<String> c) {
    c.o("/en-us/api/feeder");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("Device for feeding animals");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/feeder-999.png");
  }
  ...
  protected void _pageUriFeeder(Wrap<String> c) {
      c.o("/en-us/search/feeder");
  }
}
