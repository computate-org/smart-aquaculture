...
public class FeedEnUSGenApiServiceImpl extends BaseApiServiceImpl implements FeedEnUSGenApiService {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			webClient.post(
					...
					.sendForm(MultiMap.caseInsensitiveMultiMap()
							...
							.add("permission", String.format("%s#%s", Feed.CLASS_SIMPLE_NAME, config.getString(ComputateConfigKeys.AUTH_SCOPE_ADMIN)))
							.add("permission", String.format("%s#%s", Feed.CLASS_SIMPLE_NAME, config.getString(ComputateConfigKeys.AUTH_SCOPE_SUPER_ADMIN)))
							.add("permission", String.format("%s#%s", Feed.CLASS_SIMPLE_NAME, "GET"))
							.add("permission", String.format("%s#%s", Feed.CLASS_SIMPLE_NAME, "POST"))
							.add("permission", String.format("%s#%s", Feed.CLASS_SIMPLE_NAME, "DELETE"))
							.add("permission", String.format("%s#%s", Feed.CLASS_SIMPLE_NAME, "PATCH"))
							.add("permission", String.format("%s#%s", Feed.CLASS_SIMPLE_NAME, "PUT"))
					...
					if(!scopes.contains("PATCH")) {
						...
						searchFeedList(siteRequest, false, true, true).onSuccess(listFeed -> {
							try {
								...
								apiRequest.setId(Optional.ofNullable(listFeed.first()).map(o2 -> o2.getEntityShortId()).orElse(null));
	...
	public Future<Feed> patchFeedFuture(Feed o, Boolean entityShortId) {
	...
	public Future<Feed> sqlPATCHFeed(Feed o, Boolean entityShortId) {
		Promise<Feed> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
	...
	public Future<Feed> sqlPOSTFeed(Feed o, Boolean entityShortId) {
		Promise<Feed> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
