...
import com.example.site.model.fiware.feeder.FeederPage;
...
public class FeederEnUSGenApiServiceImpl extends BaseApiServiceImpl implements FeederEnUSGenApiService {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			webClient.post(
					...
					.sendForm(MultiMap.caseInsensitiveMultiMap()
							...
							.add("permission", String.format("%s#%s", Feeder.CLASS_SIMPLE_NAME, config.getString(ComputateConfigKeys.AUTH_SCOPE_ADMIN)))
							.add("permission", String.format("%s#%s", Feeder.CLASS_SIMPLE_NAME, config.getString(ComputateConfigKeys.AUTH_SCOPE_SUPER_ADMIN)))
							.add("permission", String.format("%s#%s", Feeder.CLASS_SIMPLE_NAME, "GET"))
							.add("permission", String.format("%s#%s", Feeder.CLASS_SIMPLE_NAME, "POST"))
							.add("permission", String.format("%s#%s", Feeder.CLASS_SIMPLE_NAME, "DELETE"))
							.add("permission", String.format("%s#%s", Feeder.CLASS_SIMPLE_NAME, "PATCH"))
							.add("permission", String.format("%s#%s", Feeder.CLASS_SIMPLE_NAME, "PUT"))
					...
					if(!scopes.contains("PATCH")) {
						...
						searchFeederList(siteRequest, false, true, true).onSuccess(listFeeder -> {
							try {
								...
								apiRequest.setId(Optional.ofNullable(listFeeder.first()).map(o2 -> o2.getEntityShortId()).orElse(null));
	...
	public Future<Feeder> patchFeederFuture(Feeder o, Boolean entityShortId) {
				...
				varsFeeder(siteRequest).onSuccess(a -> {
					...
					if(config.getBoolean(ComputateConfigKeys.ENABLE_CONTEXT_BROKER_SEND)) {
						ngsildGetEntity(o).compose(ngsildData -> {
							...
							sqlPATCHFeeder(o, entityShortId).onSuccess(feeder -> {
	...
	public Future<Feeder> sqlPATCHFeeder(Feeder o, Boolean entityShortId) {
		Promise<Feeder> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
	...
	public Future<Feeder> sqlPOSTFeeder(Feeder o, Boolean entityShortId) {
		Promise<Feeder> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
