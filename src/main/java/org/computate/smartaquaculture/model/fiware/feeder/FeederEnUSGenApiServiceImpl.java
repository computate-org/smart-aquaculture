...
public class FeederEnUSGenApiServiceImpl extends BaseApiServiceImpl implements FeederEnUSGenApiService {
  ...
  public void searchpagefrfrFeeder(ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
    ...
    user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
      try {
        ...
        String FEEDER = siteRequest.getServiceRequest().getParams().getJsonObject("path").getString("FEEDER");
        List<String> groups = Optional.ofNullable(siteRequest.getGroups()).orElse(new ArrayList<>());
        MultiMap form = MultiMap.caseInsensitiveMultiMap();
        form.add("grant_type", "urn:ietf:params:oauth:grant-type:uma-ticket");
        form.add("audience", config.getString(ComputateConfigKeys.AUTH_CLIENT));
        form.add("response_mode", "permissions");
        form.add("permission", String.format("%s#%s", Feeder.CLASS_AUTH_RESOURCE, "GET"));
        form.add("permission", String.format("%s#%s", Feeder.CLASS_AUTH_RESOURCE, "POST"));
        form.add("permission", String.format("%s#%s", Feeder.CLASS_AUTH_RESOURCE, "PATCH"));
        form.add("permission", String.format("%s#%s", Feeder.CLASS_AUTH_RESOURCE, "PUT"));
        form.add("permission", String.format("%s#%s", Feeder.CLASS_AUTH_RESOURCE, "DELETE"));
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            JsonArray scopes = authorizationDecisionBody.stream().map(o -> (JsonObject)o).filter(o -> "FEEDER".equals(o.getString("rsname"))).findFirst().map(decision -> ((JsonObject)decision).getJsonArray("scopes")).orElse(new JsonArray());
            {
              siteRequest.setScopes(scopes.stream().map(o -> o.toString()).collect(Collectors.toList()));
              List<String> scopes2 = siteRequest.getScopes();
              searchFeederList(siteRequest, false, true, false, "GET").onSuccess(listFeeder -> {
                response200SearchPageFrFRFeeder(listFeeder).onSuccess(response -> {
                  eventHandler.handle(Future.succeededFuture(response));
                  LOG.debug(String.format("searchpagefrfrFeeder succeeded. "));
                }).onFailure(ex -> {
                  LOG.error(String.format("searchpagefrfrFeeder failed. "), ex);
      ...
      if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
        try {
          ...
          LOG.error(String.format("searchpagefrfrFeeder failed. ", ex2));
  ...
  public void searchpagefrfrFeederPageInit(JsonObject ctx, FeederPage page, SearchList<Feeder> listFeeder, Promise<Void> promise) {
    String siteBaseUrl = config.getString(ComputateConfigKeys.SITE_BASE_URL);

    ctx.put("frFRUrlSearchPage", String.format("%s%s", siteBaseUrl, "/fr-fr/rechercher/nourrisseur"));
    ctx.put("frFRUrlPage", String.format("%s%s", siteBaseUrl, "/fr-fr/rechercher/nourrisseur"));
    ...
    ctx.put("enUSUrlSearchPage", String.format("%s%s", siteBaseUrl, "/en-us/search/feeder"));
    ctx.put("enUSUrlPage", String.format("%s%s", siteBaseUrl, "/en-us/search/feeder"));
  ...
  public String templateUriSearchPageFrFRFeeder(ServiceRequest serviceRequest, Feeder result) {
    return "fr-fr/rechercher/nourrisseur/FeederSearchPage.htm";
  }
  public void templateSearchPageFrFRFeeder(JsonObject ctx, FeederPage page, SearchList<Feeder> listFeeder, Promise<String> promise) {
    try {
      SiteRequest siteRequest = listFeeder.getSiteRequest_(SiteRequest.class);
      ServiceRequest serviceRequest = siteRequest.getServiceRequest();
      Feeder result = listFeeder.first();
      String pageTemplateUri = templateUriSearchPageFrFRFeeder(serviceRequest, result);
      String siteTemplatePath = config.getString(ComputateConfigKeys.TEMPLATE_PATH);
      Path resourceTemplatePath = Path.of(siteTemplatePath, pageTemplateUri);
      if(result == null || !Files.exists(resourceTemplatePath)) {
        String template = Files.readString(Path.of(siteTemplatePath, "fr-fr/rechercher/nourrisseur/FeederSearchPage.htm"), Charset.forName("UTF-8"));
      ...
      LOG.error(String.format("templateSearchPageFrFRFeeder failed. "), ex);
      ExceptionUtils.rethrow(ex);
    }
  }
  public Future<ServiceResponse> response200SearchPageFrFRFeeder(SearchList<Feeder> listFeeder) {
    ...
    try {
      ...
      page.promiseDeepFeederPage(siteRequest).onSuccess(a -> {
        try {
          ...
          searchpagefrfrFeederPageInit(ctx, page, listFeeder, promise1);
          promise1.future().onSuccess(b -> {
            Promise<String> promise2 = Promise.promise();
            templateSearchPageFrFRFeeder(ctx, page, listFeeder, promise2);
            promise2.future().onSuccess(renderedTemplate -> {
              try {
                Buffer buffer = Buffer.buffer(renderedTemplate);
                promise.complete(new ServiceResponse(200, "OK", buffer, requestHeaders));
              } catch(Throwable ex) {
                LOG.error(String.format("response200SearchPageFrFRFeeder failed. "), ex);
                promise.fail(ex);
              }
            }).onFailure(ex -> {
              promise.fail(ex);
  ...
  public void responsePivotSearchPageFrFRFeeder(List<SolrResponse.Pivot> pivots, JsonArray pivotArray) {
    if(pivots != null) {
      for(SolrResponse.Pivot pivotField : pivots) {
        ...
        if(pivotFields2 != null) {
          ...
          responsePivotSearchPageFrFRFeeder(pivotFields2, pivotArray2);
        }
      }
    }
  }
    ...
    user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            JsonArray scopes = authorizationDecisionBody.stream().map(o -> (JsonObject)o).filter(o -> "FEEDER".equals(o.getString("rsname"))).findFirst().map(decision -> ((JsonObject)decision).getJsonArray("scopes")).orElse(new JsonArray());
            {
              ...
              List<String> scopes2 = siteRequest.getScopes();
              searchFeederList(siteRequest, false, true, false, "GET").onSuccess(listFeeder -> {
                response200EditPageFrFRFeeder(listFeeder).onSuccess(response -> {
                  eventHandler.handle(Future.succeededFuture(response));
                  LOG.debug(String.format("editpagefrfrFeeder succeeded. "));
                }).onFailure(ex -> {
                  LOG.error(String.format("editpagefrfrFeeder failed. "), ex);
                  error(siteRequest, eventHandler, ex);
                });
              }).onFailure(ex -> {
                LOG.error(String.format("editpagefrfrFeeder failed. "), ex);
      ...
      if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
        try {
          ...
          LOG.error(String.format("editpagefrfrFeeder failed. ", ex2));
          error(null, eventHandler, ex2);
        }
      } else if(StringUtils.startsWith(ex.getMessage(), "401 UNAUTHORIZED ")) {
        eventHandler.handle(Future.succeededFuture(
  ...
  public void editpagefrfrFeederPageInit(JsonObject ctx, FeederPage page, SearchList<Feeder> listFeeder, Promise<Void> promise) {
    String siteBaseUrl = config.getString(ComputateConfigKeys.SITE_BASE_URL);
  ...
  public String templateUriEditPageFrFRFeeder(ServiceRequest serviceRequest, Feeder result) {
    return "fr-fr/edition/nourrisseur/FeederEditPage.htm";
  }
  public void templateEditPageFrFRFeeder(JsonObject ctx, FeederPage page, SearchList<Feeder> listFeeder, Promise<String> promise) {
    try {
      SiteRequest siteRequest = listFeeder.getSiteRequest_(SiteRequest.class);
      ServiceRequest serviceRequest = siteRequest.getServiceRequest();
      Feeder result = listFeeder.first();
      String pageTemplateUri = templateUriEditPageFrFRFeeder(serviceRequest, result);
      String siteTemplatePath = config.getString(ComputateConfigKeys.TEMPLATE_PATH);
      Path resourceTemplatePath = Path.of(siteTemplatePath, pageTemplateUri);
      if(result == null || !Files.exists(resourceTemplatePath)) {
        String template = Files.readString(Path.of(siteTemplatePath, "fr-fr/edition/nourrisseur/FeederEditPage.htm"), Charset.forName("UTF-8"));
        String renderedTemplate = jinjava.render(template, ctx.getMap());
        promise.complete(renderedTemplate);
      } else if(pageTemplateUri.endsWith(".md")) {
        String template = siteTemplatePath == null ? Resources.toString(Resources.getResource(resourceTemplatePath.toString()), StandardCharsets.UTF_8) : Files.readString(resourceTemplatePath, Charset.forName("UTF-8"));
      ...
      LOG.error(String.format("templateEditPageFrFRFeeder failed. "), ex);
      ExceptionUtils.rethrow(ex);
    }
  }
  public Future<ServiceResponse> response200EditPageFrFRFeeder(SearchList<Feeder> listFeeder) {
    Promise<ServiceResponse> promise = Promise.promise();
    try {
      SiteRequest siteRequest = listFeeder.getSiteRequest_(SiteRequest.class);
      FeederPage page = new FeederPage();
      ...
      page.promiseDeepFeederPage(siteRequest).onSuccess(a -> {
        try {
          ...
          ctx.mergeIn(JsonObject.mapFrom(page));
          Promise<Void> promise1 = Promise.promise();
          editpagefrfrFeederPageInit(ctx, page, listFeeder, promise1);
          promise1.future().onSuccess(b -> {
            Promise<String> promise2 = Promise.promise();
            templateEditPageFrFRFeeder(ctx, page, listFeeder, promise2);
            promise2.future().onSuccess(renderedTemplate -> {
              try {
                Buffer buffer = Buffer.buffer(renderedTemplate);
                promise.complete(new ServiceResponse(200, "OK", buffer, requestHeaders));
              } catch(Throwable ex) {
                LOG.error(String.format("response200EditPageFrFRFeeder failed. "), ex);
                promise.fail(ex);
              }
            }).onFailure(ex -> {
              promise.fail(ex);
  ...
  public void responsePivotEditPageFrFRFeeder(List<SolrResponse.Pivot> pivots, JsonArray pivotArray) {
    if(pivots != null) {
      for(SolrResponse.Pivot pivotField : pivots) {
        String entityIndexed = pivotField.getField();
        String entityVar = StringUtils.substringBefore(entityIndexed, "_docvalues_");
        ...
        if(pivotFields2 != null) {
          JsonArray pivotArray2 = new JsonArray();
          pivotJson.put("pivot", pivotArray2);
          responsePivotEditPageFrFRFeeder(pivotFields2, pivotArray2);
        }
      }
    }
  }
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            if(authorizationDecisionResponse.failed() || !scopes.contains("PATCH")) {
              ...
              siteRequest.setScopes(scopes.stream().map(o -> o.toString()).collect(Collectors.toList()));
              List<String> scopes2 = siteRequest.getScopes();
              searchFeederList(siteRequest, false, true, true, "PATCH").onSuccess(listFeeder -> {
                try {
                  ...
                  apiRequest.setSolrId(Optional.ofNullable(listFeeder.first()).map(o2 -> o2.getSolrId()).orElse(null));
        ...
        searchFeederList(siteRequest, false, true, true, "PATCH").onSuccess(listFeeder -> {
          try {
            ...
            if(o != null) {
              ...
              apiRequest.setId(Optional.ofNullable(listFeeder.first()).map(o3 -> o3.getEntityShortId().toString()).orElse(null));
              apiRequest.setSolrId(Optional.ofNullable(listFeeder.first()).map(o3 -> o3.getSolrId()).orElse(null));
              JsonObject jsonObject = JsonObject.mapFrom(o);
              o2 = jsonObject.mapTo(Feeder.class);
              ...
              String m = String.format("%s %s not found", "Feeder", null);
      ...
      for(String entityVar : methodNames) {
        switch(entityVar) {
          ...
          case "setAreaServed":
              ...
              if(bParams.size() > 0)
                bSql.append(", ");
              bSql.append(String.format("%s=ST_GeomFromGeoJSON($%s)", Feeder.VAR_areaServed, num));
              num++;
          ...
          try {
            ...
            if(authorizationDecisionResponse.failed() || !scopes.contains("POST")) {
              ...
              eventBus.request(Feeder.getClassApiAddress(), json, new DeliveryOptions().addHeader("action", "postFeederFuture")).onSuccess(a -> {
                ...
                apiRequest.setSolrId(responseBody.getString(Feeder.VAR_solrId));
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            if(authorizationDecisionResponse.failed() || !scopes.contains("DELETE")) {
              ...
              siteRequest.setScopes(scopes.stream().map(o -> o.toString()).collect(Collectors.toList()));
              List<String> scopes2 = siteRequest.getScopes();
              searchFeederList(siteRequest, false, true, true, "DELETE").onSuccess(listFeeder -> {
                try {
                  ApiRequest apiRequest = new ApiRequest();
                  apiRequest.setRows(listFeeder.getRequest().getRows());
                  apiRequest.setNumFound(listFeeder.getResponse().getResponse().getNumFound());
  ...
  public void searchpageFeederPageInit(JsonObject ctx, FeederPage page, SearchList<Feeder> listFeeder, Promise<Void> promise) {
    String siteBaseUrl = config.getString(ComputateConfigKeys.SITE_BASE_URL);
  ...
  public String templateUriSearchPageFeeder(ServiceRequest serviceRequest, Feeder result) {
    return "en-us/search/feeder/FeederSearchPage.htm";
  }
  public void templateSearchPageFeeder(JsonObject ctx, FeederPage page, SearchList<Feeder> listFeeder, Promise<String> promise) {
    try {
      SiteRequest siteRequest = listFeeder.getSiteRequest_(SiteRequest.class);
      ServiceRequest serviceRequest = siteRequest.getServiceRequest();
      Feeder result = listFeeder.first();
      String pageTemplateUri = templateUriSearchPageFeeder(serviceRequest, result);
      String siteTemplatePath = config.getString(ComputateConfigKeys.TEMPLATE_PATH);
      Path resourceTemplatePath = Path.of(siteTemplatePath, pageTemplateUri);
      if(result == null || !Files.exists(resourceTemplatePath)) {
        String template = Files.readString(Path.of(siteTemplatePath, "en-us/search/feeder/FeederSearchPage.htm"), Charset.forName("UTF-8"));
        String renderedTemplate = jinjava.render(template, ctx.getMap());
        promise.complete(renderedTemplate);
      } else if(pageTemplateUri.endsWith(".md")) {
        String template = siteTemplatePath == null ? Resources.toString(Resources.getResource(resourceTemplatePath.toString()), StandardCharsets.UTF_8) : Files.readString(resourceTemplatePath, Charset.forName("UTF-8"));
      ...
      LOG.error(String.format("templateSearchPageFeeder failed. "), ex);
      ExceptionUtils.rethrow(ex);
    }
  }
    ...
    try {
      ...
      page.promiseDeepFeederPage(siteRequest).onSuccess(a -> {
        try {
          ...
          ctx.mergeIn(JsonObject.mapFrom(page));
          Promise<Void> promise1 = Promise.promise();
          searchpageFeederPageInit(ctx, page, listFeeder, promise1);
          promise1.future().onSuccess(b -> {
            Promise<String> promise2 = Promise.promise();
            templateSearchPageFeeder(ctx, page, listFeeder, promise2);
            promise2.future().onSuccess(renderedTemplate -> {
              try {
                Buffer buffer = Buffer.buffer(renderedTemplate);
                promise.complete(new ServiceResponse(200, "OK", buffer, requestHeaders));
  ...
  public void editpageFeederPageInit(JsonObject ctx, FeederPage page, SearchList<Feeder> listFeeder, Promise<Void> promise) {
    String siteBaseUrl = config.getString(ComputateConfigKeys.SITE_BASE_URL);
  ...
  public String templateUriEditPageFeeder(ServiceRequest serviceRequest, Feeder result) {
    return "en-us/edit/feeder/FeederEditPage.htm";
  }
  public void templateEditPageFeeder(JsonObject ctx, FeederPage page, SearchList<Feeder> listFeeder, Promise<String> promise) {
    try {
      SiteRequest siteRequest = listFeeder.getSiteRequest_(SiteRequest.class);
      ServiceRequest serviceRequest = siteRequest.getServiceRequest();
      Feeder result = listFeeder.first();
      String pageTemplateUri = templateUriEditPageFeeder(serviceRequest, result);
      String siteTemplatePath = config.getString(ComputateConfigKeys.TEMPLATE_PATH);
      Path resourceTemplatePath = Path.of(siteTemplatePath, pageTemplateUri);
      if(result == null || !Files.exists(resourceTemplatePath)) {
        String template = Files.readString(Path.of(siteTemplatePath, "en-us/edit/feeder/FeederEditPage.htm"), Charset.forName("UTF-8"));
        String renderedTemplate = jinjava.render(template, ctx.getMap());
        promise.complete(renderedTemplate);
      } else if(pageTemplateUri.endsWith(".md")) {
        String template = siteTemplatePath == null ? Resources.toString(Resources.getResource(resourceTemplatePath.toString()), StandardCharsets.UTF_8) : Files.readString(resourceTemplatePath, Charset.forName("UTF-8"));
      ...
      LOG.error(String.format("templateEditPageFeeder failed. "), ex);
      ExceptionUtils.rethrow(ex);
    }
  }
    ...
    try {
      ...
      page.promiseDeepFeederPage(siteRequest).onSuccess(a -> {
        try {
          ...
          ctx.mergeIn(JsonObject.mapFrom(page));
          Promise<Void> promise1 = Promise.promise();
          editpageFeederPageInit(ctx, page, listFeeder, promise1);
          promise1.future().onSuccess(b -> {
            Promise<String> promise2 = Promise.promise();
            templateEditPageFeeder(ctx, page, listFeeder, promise2);
            promise2.future().onSuccess(renderedTemplate -> {
              try {
                Buffer buffer = Buffer.buffer(renderedTemplate);
                promise.complete(new ServiceResponse(200, "OK", buffer, requestHeaders));
  ...
  public Future<SearchList<Feeder>> searchFeederList(SiteRequest siteRequest, Boolean populate, Boolean store, Boolean modify, String scope) {
  ...
  public Future<JsonObject> generatePageBody(ComputateSiteRequest siteRequest, Map<String, Object> ctx, String templatePath, String classSimpleName, String pageTemplate) {
    ...
    try {
      ...
      o.persistForClass(Feeder.VAR_name, Feeder.staticSetName(siteRequest2, (String)result.get(Feeder.VAR_name)));
      o.persistForClass(Feeder.VAR_description, Feeder.staticSetDescription(siteRequest2, (String)result.get(Feeder.VAR_description)));
      o.persistForClass(Feeder.VAR_created, Feeder.staticSetCreated(siteRequest2, (String)result.get(Feeder.VAR_created), Optional.ofNullable(siteRequest).map(r -> r.getConfig()).map(config -> config.getString(ConfigKeys.SITE_ZONE)).map(z -> ZoneId.of(z)).orElse(ZoneId.of("UTC"))));
      o.persistForClass(Feeder.VAR_location, Feeder.staticSetLocation(siteRequest2, (String)result.get(Feeder.VAR_location)));
      o.persistForClass(Feeder.VAR_archived, Feeder.staticSetArchived(siteRequest2, (String)result.get(Feeder.VAR_archived)));
      o.persistForClass(Feeder.VAR_areaServed, Feeder.staticSetAreaServed(siteRequest2, (String)result.get(Feeder.VAR_areaServed)));
      o.persistForClass(Feeder.VAR_id, Feeder.staticSetId(siteRequest2, (String)result.get(Feeder.VAR_id)));
      o.persistForClass(Feeder.VAR_sessionId, Feeder.staticSetSessionId(siteRequest2, (String)result.get(Feeder.VAR_sessionId)));
      o.persistForClass(Feeder.VAR_userKey, Feeder.staticSetUserKey(siteRequest2, (String)result.get(Feeder.VAR_userKey)));
      o.persistForClass(Feeder.VAR_ngsildTenant, Feeder.staticSetNgsildTenant(siteRequest2, (String)result.get(Feeder.VAR_ngsildTenant)));
      o.persistForClass(Feeder.VAR_ngsildPath, Feeder.staticSetNgsildPath(siteRequest2, (String)result.get(Feeder.VAR_ngsildPath)));
      o.persistForClass(Feeder.VAR_ngsildContext, Feeder.staticSetNgsildContext(siteRequest2, (String)result.get(Feeder.VAR_ngsildContext)));
      o.persistForClass(Feeder.VAR_objectTitle, Feeder.staticSetObjectTitle(siteRequest2, (String)result.get(Feeder.VAR_objectTitle)));
      o.persistForClass(Feeder.VAR_ngsildData, Feeder.staticSetNgsildData(siteRequest2, (String)result.get(Feeder.VAR_ngsildData)));
      o.persistForClass(Feeder.VAR_displayPage, Feeder.staticSetDisplayPage(siteRequest2, (String)result.get(Feeder.VAR_displayPage)));
      o.persistForClass(Feeder.VAR_address, Feeder.staticSetAddress(siteRequest2, (String)result.get(Feeder.VAR_address)));
      o.persistForClass(Feeder.VAR_displayPageFrFR, Feeder.staticSetDisplayPageFrFR(siteRequest2, (String)result.get(Feeder.VAR_displayPageFrFR)));
      o.persistForClass(Feeder.VAR_alternateName, Feeder.staticSetAlternateName(siteRequest2, (String)result.get(Feeder.VAR_alternateName)));
      o.persistForClass(Feeder.VAR_editPage, Feeder.staticSetEditPage(siteRequest2, (String)result.get(Feeder.VAR_editPage)));
      o.persistForClass(Feeder.VAR_dataProvider, Feeder.staticSetDataProvider(siteRequest2, (String)result.get(Feeder.VAR_dataProvider)));
      o.persistForClass(Feeder.VAR_editPageFrFR, Feeder.staticSetEditPageFrFR(siteRequest2, (String)result.get(Feeder.VAR_editPageFrFR)));
      o.persistForClass(Feeder.VAR_dateCreated, Feeder.staticSetDateCreated(siteRequest2, (String)result.get(Feeder.VAR_dateCreated)));
      o.persistForClass(Feeder.VAR_userPage, Feeder.staticSetUserPage(siteRequest2, (String)result.get(Feeder.VAR_userPage)));
      o.persistForClass(Feeder.VAR_dateModified, Feeder.staticSetDateModified(siteRequest2, (String)result.get(Feeder.VAR_dateModified)));
      o.persistForClass(Feeder.VAR_userPageFrFR, Feeder.staticSetUserPageFrFR(siteRequest2, (String)result.get(Feeder.VAR_userPageFrFR)));
      o.persistForClass(Feeder.VAR_owner, Feeder.staticSetOwner(siteRequest2, (String)result.get(Feeder.VAR_owner)));
      o.persistForClass(Feeder.VAR_download, Feeder.staticSetDownload(siteRequest2, (String)result.get(Feeder.VAR_download)));
      o.persistForClass(Feeder.VAR_relatedSource, Feeder.staticSetRelatedSource(siteRequest2, (String)result.get(Feeder.VAR_relatedSource)));
      o.persistForClass(Feeder.VAR_downloadFrFR, Feeder.staticSetDownloadFrFR(siteRequest2, (String)result.get(Feeder.VAR_downloadFrFR)));
      o.persistForClass(Feeder.VAR_seeAlso, Feeder.staticSetSeeAlso(siteRequest2, (String)result.get(Feeder.VAR_seeAlso)));
      o.persistForClass(Feeder.VAR_source, Feeder.staticSetSource(siteRequest2, (String)result.get(Feeder.VAR_source)));
