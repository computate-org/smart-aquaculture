...
public class FeedEnUSGenApiServiceImpl extends BaseApiServiceImpl implements FeedEnUSGenApiService {
  ...
  @Override
  public void searchpagefrfrFeed(ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
    ...
    user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
      try {
        ...
        String FEED = siteRequest.getServiceRequest().getParams().getJsonObject("path").getString("FEED");
        List<String> groups = Optional.ofNullable(siteRequest.getGroups()).orElse(new ArrayList<>());
        MultiMap form = MultiMap.caseInsensitiveMultiMap();
        form.add("grant_type", "urn:ietf:params:oauth:grant-type:uma-ticket");
        form.add("audience", config.getString(ComputateConfigKeys.AUTH_CLIENT));
        form.add("response_mode", "permissions");
        form.add("permission", String.format("%s#%s", Feed.CLASS_AUTH_RESOURCE, "GET"));
        form.add("permission", String.format("%s#%s", Feed.CLASS_AUTH_RESOURCE, "POST"));
        ...
        form.add("permission", String.format("%s#%s", Feed.CLASS_AUTH_RESOURCE, "SuperAdmin"));
        form.add("permission", String.format("%s#%s", Feed.CLASS_AUTH_RESOURCE, "Admin"));
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            JsonArray scopes = authorizationDecisionBody.stream().map(o -> (JsonObject)o).filter(o -> "FEED".equals(o.getString("rsname"))).findFirst().map(decision -> ((JsonObject)decision).getJsonArray("scopes")).orElse(new JsonArray());
            {
              siteRequest.setScopes(scopes.stream().map(o -> o.toString()).collect(Collectors.toList()));
              List<String> scopes2 = siteRequest.getScopes();
              searchFeedList(siteRequest, false, true, false, "GET").onSuccess(listFeed -> {
                response200SearchPageFrFRFeed(listFeed).onSuccess(response -> {
                  eventHandler.handle(Future.succeededFuture(response));
                  LOG.debug(String.format("searchpagefrfrFeed succeeded. "));
                }).onFailure(ex -> {
                  LOG.error(String.format("searchpagefrfrFeed failed. "), ex);
      ...
      if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
        try {
          ...
          LOG.error(String.format("searchpagefrfrFeed failed. ", ex2));
  ...
  public void searchpagefrfrFeedPageInit(JsonObject ctx, FeedPage page, SearchList<Feed> listFeed, Promise<Void> promise) {
    String siteBaseUrl = config.getString(ComputateConfigKeys.SITE_BASE_URL);

    ctx.put("frFRUrlSearchPage", String.format("%s%s", siteBaseUrl, "/fr-fr/rechercher/nourriture"));
    ctx.put("frFRUrlPage", String.format("%s%s", siteBaseUrl, "/fr-fr/rechercher/nourriture"));
    ...
    ctx.put("enUSUrlSearchPage", String.format("%s%s", siteBaseUrl, "/en-us/search/feed"));
    ctx.put("enUSUrlPage", String.format("%s%s", siteBaseUrl, "/en-us/search/feed"));
  ...
  public String templateUriSearchPageFrFRFeed(ServiceRequest serviceRequest, Feed result) {
    return "fr-fr/rechercher/nourriture/FeedSearchPage.htm";
  }
  public void templateSearchPageFrFRFeed(JsonObject ctx, FeedPage page, SearchList<Feed> listFeed, Promise<String> promise) {
    try {
      SiteRequest siteRequest = listFeed.getSiteRequest_(SiteRequest.class);
      ServiceRequest serviceRequest = siteRequest.getServiceRequest();
      Feed result = listFeed.first();
      String pageTemplateUri = templateUriSearchPageFrFRFeed(serviceRequest, result);
      String siteTemplatePath = config.getString(ComputateConfigKeys.TEMPLATE_PATH);
      Path resourceTemplatePath = Path.of(siteTemplatePath, pageTemplateUri);
      if(result == null || !Files.exists(resourceTemplatePath)) {
        String template = Files.readString(Path.of(siteTemplatePath, "fr-fr/rechercher/nourriture/FeedSearchPage.htm"), Charset.forName("UTF-8"));
      ...
      LOG.error(String.format("templateSearchPageFrFRFeed failed. "), ex);
      ExceptionUtils.rethrow(ex);
    }
  }
  public Future<ServiceResponse> response200SearchPageFrFRFeed(SearchList<Feed> listFeed) {
    ...
    try {
      ...
      page.promiseDeepFeedPage(siteRequest).onSuccess(a -> {
        try {
          ...
          searchpagefrfrFeedPageInit(ctx, page, listFeed, promise1);
          promise1.future().onSuccess(b -> {
            Promise<String> promise2 = Promise.promise();
            templateSearchPageFrFRFeed(ctx, page, listFeed, promise2);
            promise2.future().onSuccess(renderedTemplate -> {
              try {
                Buffer buffer = Buffer.buffer(renderedTemplate);
                promise.complete(new ServiceResponse(200, "OK", buffer, requestHeaders));
              } catch(Throwable ex) {
                LOG.error(String.format("response200SearchPageFrFRFeed failed. "), ex);
                promise.fail(ex);
              }
            }).onFailure(ex -> {
              promise.fail(ex);
  ...
  public void responsePivotSearchPageFrFRFeed(List<SolrResponse.Pivot> pivots, JsonArray pivotArray) {
    if(pivots != null) {
      for(SolrResponse.Pivot pivotField : pivots) {
        ...
        if(pivotFields2 != null) {
          ...
          responsePivotSearchPageFrFRFeed(pivotFields2, pivotArray2);
        }
      }
    }
  }
  ...
  @Override
  public void editpagefrfrFeed(ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
    Boolean classPublicRead = false;
    user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
      try {
        siteRequest.setLang("frFR");
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            JsonArray scopes = authorizationDecisionBody.stream().map(o -> (JsonObject)o).filter(o -> "FEED".equals(o.getString("rsname"))).findFirst().map(decision -> ((JsonObject)decision).getJsonArray("scopes")).orElse(new JsonArray());
            {
              ...
              List<String> scopes2 = siteRequest.getScopes();
              searchFeedList(siteRequest, false, true, false, "GET").onSuccess(listFeed -> {
                response200EditPageFrFRFeed(listFeed).onSuccess(response -> {
                  eventHandler.handle(Future.succeededFuture(response));
                  LOG.debug(String.format("editpagefrfrFeed succeeded. "));
                }).onFailure(ex -> {
                  LOG.error(String.format("editpagefrfrFeed failed. "), ex);
                  error(siteRequest, eventHandler, ex);
                });
              }).onFailure(ex -> {
                LOG.error(String.format("editpagefrfrFeed failed. "), ex);
      ...
      if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
        try {
          ...
          LOG.error(String.format("editpagefrfrFeed failed. ", ex2));
          error(null, eventHandler, ex2);
        }
      } else if(StringUtils.startsWith(ex.getMessage(), "401 UNAUTHORIZED ")) {
        eventHandler.handle(Future.succeededFuture(
  ...
  public void editpagefrfrFeedPageInit(JsonObject ctx, FeedPage page, SearchList<Feed> listFeed, Promise<Void> promise) {
    String siteBaseUrl = config.getString(ComputateConfigKeys.SITE_BASE_URL);
  ...
  public String templateUriEditPageFrFRFeed(ServiceRequest serviceRequest, Feed result) {
    return "fr-fr/edition/nourriture/FeedEditPage.htm";
  }
  public void templateEditPageFrFRFeed(JsonObject ctx, FeedPage page, SearchList<Feed> listFeed, Promise<String> promise) {
    try {
      SiteRequest siteRequest = listFeed.getSiteRequest_(SiteRequest.class);
      ServiceRequest serviceRequest = siteRequest.getServiceRequest();
      Feed result = listFeed.first();
      String pageTemplateUri = templateUriEditPageFrFRFeed(serviceRequest, result);
      String siteTemplatePath = config.getString(ComputateConfigKeys.TEMPLATE_PATH);
      Path resourceTemplatePath = Path.of(siteTemplatePath, pageTemplateUri);
      if(result == null || !Files.exists(resourceTemplatePath)) {
        String template = Files.readString(Path.of(siteTemplatePath, "fr-fr/edition/nourriture/FeedEditPage.htm"), Charset.forName("UTF-8"));
        String renderedTemplate = jinjava.render(template, ctx.getMap());
        promise.complete(renderedTemplate);
      } else if(pageTemplateUri.endsWith(".md")) {
        String template = siteTemplatePath == null ? Resources.toString(Resources.getResource(resourceTemplatePath.toString()), StandardCharsets.UTF_8) : Files.readString(resourceTemplatePath, Charset.forName("UTF-8"));
      ...
      LOG.error(String.format("templateEditPageFrFRFeed failed. "), ex);
      ExceptionUtils.rethrow(ex);
    }
  }
  public Future<ServiceResponse> response200EditPageFrFRFeed(SearchList<Feed> listFeed) {
    Promise<ServiceResponse> promise = Promise.promise();
    try {
      SiteRequest siteRequest = listFeed.getSiteRequest_(SiteRequest.class);
      FeedPage page = new FeedPage();
      ...
      page.promiseDeepFeedPage(siteRequest).onSuccess(a -> {
        try {
          ...
          ctx.mergeIn(JsonObject.mapFrom(page));
          Promise<Void> promise1 = Promise.promise();
          editpagefrfrFeedPageInit(ctx, page, listFeed, promise1);
          promise1.future().onSuccess(b -> {
            Promise<String> promise2 = Promise.promise();
            templateEditPageFrFRFeed(ctx, page, listFeed, promise2);
            promise2.future().onSuccess(renderedTemplate -> {
              try {
                Buffer buffer = Buffer.buffer(renderedTemplate);
                promise.complete(new ServiceResponse(200, "OK", buffer, requestHeaders));
              } catch(Throwable ex) {
                LOG.error(String.format("response200EditPageFrFRFeed failed. "), ex);
                promise.fail(ex);
              }
            }).onFailure(ex -> {
              promise.fail(ex);
  ...
  public void responsePivotEditPageFrFRFeed(List<SolrResponse.Pivot> pivots, JsonArray pivotArray) {
    if(pivots != null) {
      for(SolrResponse.Pivot pivotField : pivots) {
        String entityIndexed = pivotField.getField();
        String entityVar = StringUtils.substringBefore(entityIndexed, "_docvalues_");
        ...
        if(pivotFields2 != null) {
          JsonArray pivotArray2 = new JsonArray();
          pivotJson.put("pivot", pivotArray2);
          responsePivotEditPageFrFRFeed(pivotFields2, pivotArray2);
        }
      }
    }
  }
  ...
  public void patchFeed(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
    ...
    user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
      try {
        ...
        form.add("permission", String.format("%s#%s", Feed.CLASS_AUTH_RESOURCE, "Admin"));
        if(entityShortId != null)
          form.add("permission", String.format("%s-%s#%s", Feed.CLASS_AUTH_RESOURCE, entityShortId, "PATCH"));
        webClient.post(
            config.getInteger(ComputateConfigKeys.AUTH_PORT)
            , config.getString(ComputateConfigKeys.AUTH_HOST_NAME)
            , config.getString(ComputateConfigKeys.AUTH_TOKEN_URI)
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            if(authorizationDecisionResponse.failed() || !scopes.contains("PATCH")) {
              ...
              siteRequest.setScopes(scopes.stream().map(o -> o.toString()).collect(Collectors.toList()));
              List<String> scopes2 = siteRequest.getScopes();
              searchFeedList(siteRequest, false, true, true, "PATCH").onSuccess(listFeed -> {
                try {
      ...
      for(String entityVar : methodNames) {
        switch(entityVar) {
          ...
          case "setEditPage":
              ...
              if(bParams.size() > 0)
                bSql.append(", ");
              bSql.append(Feed.VAR_editPage + "=$" + num);
              num++;
  ...
  public void postFeed(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
    ...
    user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
      try {
        ...
        form.add("permission", String.format("%s#%s", Feed.CLASS_AUTH_RESOURCE, "Admin"));
        if(entityShortId != null)
          form.add("permission", String.format("%s-%s#%s", Feed.CLASS_AUTH_RESOURCE, entityShortId, "POST"));
        webClient.post(
            config.getInteger(ComputateConfigKeys.AUTH_PORT)
            , config.getString(ComputateConfigKeys.AUTH_HOST_NAME)
            , config.getString(ComputateConfigKeys.AUTH_TOKEN_URI)
  ...
  public void deleteFeed(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
    ...
    user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
      try {
        ...
        form.add("permission", String.format("%s#%s", Feed.CLASS_AUTH_RESOURCE, "Admin"));
        if(entityShortId != null)
          form.add("permission", String.format("%s-%s#%s", Feed.CLASS_AUTH_RESOURCE, entityShortId, "DELETE"));
        webClient.post(
            config.getInteger(ComputateConfigKeys.AUTH_PORT)
            , config.getString(ComputateConfigKeys.AUTH_HOST_NAME)
            , config.getString(ComputateConfigKeys.AUTH_TOKEN_URI)
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            if(authorizationDecisionResponse.failed() || !scopes.contains("DELETE")) {
              ...
              siteRequest.setScopes(scopes.stream().map(o -> o.toString()).collect(Collectors.toList()));
              List<String> scopes2 = siteRequest.getScopes();
              searchFeedList(siteRequest, false, true, true, "DELETE").onSuccess(listFeed -> {
                try {
                  ApiRequest apiRequest = new ApiRequest();
                  apiRequest.setRows(listFeed.getRequest().getRows());
                  apiRequest.setNumFound(listFeed.getResponse().getResponse().getNumFound());
  ...
  public void putimportFeed(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
    ...
    user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
      try {
        ...
        form.add("permission", String.format("%s#%s", Feed.CLASS_AUTH_RESOURCE, "Admin"));
        if(entityShortId != null)
          form.add("permission", String.format("%s-%s#%s", Feed.CLASS_AUTH_RESOURCE, entityShortId, "PUT"));
        webClient.post(
            config.getInteger(ComputateConfigKeys.AUTH_PORT)
            , config.getString(ComputateConfigKeys.AUTH_HOST_NAME)
            , config.getString(ComputateConfigKeys.AUTH_TOKEN_URI)
  ...
  public void templateSearchPageFeed(JsonObject ctx, FeedPage page, SearchList<Feed> listFeed, Promise<String> promise) {
    try {
      ...
      Path resourceTemplatePath = Path.of(siteTemplatePath, pageTemplateUri);
      if(result == null || !Files.exists(resourceTemplatePath)) {
        String template = Files.readString(Path.of(siteTemplatePath, "en-us/search/feed/FeedSearchPage.htm"), Charset.forName("UTF-8"));
        String renderedTemplate = jinjava.render(template, ctx.getMap());
        promise.complete(renderedTemplate);
      } else if(pageTemplateUri.endsWith(".md")) {
        String template = siteTemplatePath == null ? Resources.toString(Resources.getResource(resourceTemplatePath.toString()), StandardCharsets.UTF_8) : Files.readString(resourceTemplatePath, Charset.forName("UTF-8"));
  ...
  public void templateEditPageFeed(JsonObject ctx, FeedPage page, SearchList<Feed> listFeed, Promise<String> promise) {
    try {
      ...
      Path resourceTemplatePath = Path.of(siteTemplatePath, pageTemplateUri);
      if(result == null || !Files.exists(resourceTemplatePath)) {
        String template = Files.readString(Path.of(siteTemplatePath, "en-us/edit/feed/FeedEditPage.htm"), Charset.forName("UTF-8"));
        String renderedTemplate = jinjava.render(template, ctx.getMap());
        promise.complete(renderedTemplate);
      } else if(pageTemplateUri.endsWith(".md")) {
        String template = siteTemplatePath == null ? Resources.toString(Resources.getResource(resourceTemplatePath.toString()), StandardCharsets.UTF_8) : Files.readString(resourceTemplatePath, Charset.forName("UTF-8"));
  ...
  public Future<SearchList<Feed>> searchFeedList(SiteRequest siteRequest, Boolean populate, Boolean store, Boolean modify, String scope) {
