 ...
 * Generated: true
 **/
public class FeedingOperationGenPage extends FeedingOperationGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListFeedingOperation_(Wrap<SearchList<FeedingOperation>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListFeedingOperation_ != null)
      w.o(Optional.ofNullable(searchListFeedingOperation_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListFeedingOperation_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListFeedingOperation_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListFeedingOperation_.getStart().longValue();
    Long rows = searchListFeedingOperation_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListFeedingOperation_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListFeedingOperation_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    FeedingOperation.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(FeedingOperation.displayNameFeedingOperation(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(FeedingOperation.classSimpleNameFeedingOperation(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListFeedingOperation_.getRequest().getQuery()).filter(fq -> fq.startsWith(FeedingOperation.varIndexedFeedingOperation(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    for(String var : FeedingOperation.varsFqForClass()) {
      ...
      json.put("displayName", Optional.ofNullable(FeedingOperation.displayNameFeedingOperation(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(FeedingOperation.classSimpleNameFeedingOperation(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListFeedingOperation_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(FeedingOperation.varIndexedFeedingOperation(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = FeedingOperation.searchVarFeedingOperation(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    FeedingOperation.varsRangeForClass().forEach(var -> {
      String varIndexed = FeedingOperation.varIndexedFeedingOperation(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(FeedingOperation.displayNameFeedingOperation(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(FeedingOperation.classSimpleNameFeedingOperation(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListFeedingOperation_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(FeedingOperation.varIndexedFeedingOperation(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListFeedingOperation_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListFeedingOperation_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListFeedingOperation_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListFeedingOperation_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListFeedingOperation_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = FeedingOperation.searchVarFeedingOperation(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", FeedingOperation.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListFeedingOperation_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = FeedingOperation.searchVarFeedingOperation(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", FeedingOperation.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListFeedingOperation_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListFeedingOperation_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListFeedingOperation_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListFeedingOperation_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return FeedingOperation.searchVarFeedingOperation(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListFeedingOperation_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListFeedingOperation_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListFeedingOperation_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListFeedingOperation_.getFacetPivotMinCount()).orElse(0));
  }

  @Override
  protected void _DEFAULT_MAP_LOCATION(Wrap<JsonObject> w) {
    Point point = FeedingOperation.staticSetLocation(siteRequest_, Optional.ofNullable(siteRequest_.getRequestVars().get(VAR_DEFAULT_MAP_LOCATION)).orElse(siteRequest_.getConfig().getString(ConfigKeys.DEFAULT_MAP_LOCATION)));
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListFeedingOperation_.getDefaultSort()) {
      Optional.ofNullable(searchListFeedingOperation_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = FeedingOperation.searchVarFeedingOperation(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListFeedingOperation_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = FeedingOperation.searchVarFeedingOperation(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListFeedingOperation_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListFeedingOperation_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listFeedingOperation(JsonArray l) {
    Optional.ofNullable(searchListFeedingOperation_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListFeedingOperation_ == null ? 0 : searchListFeedingOperation_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<FeedingOperation> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("entityShortId")).orElse(null) != null)
      w.o(searchListFeedingOperation_.get(0));
  }
  ...
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/feeding-operation");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("Feeding operation on a group of animals");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/feeding-operation-999.png");
  }
  ...
  protected void _pageUriFeedingOperation(Wrap<String> c) {
      c.o("/en-us/search/feeding-operation");
  }
}
