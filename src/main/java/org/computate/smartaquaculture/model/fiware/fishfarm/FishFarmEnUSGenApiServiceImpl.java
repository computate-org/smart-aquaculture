...
public class FishFarmEnUSGenApiServiceImpl extends BaseApiServiceImpl implements FishFarmEnUSGenApiService {
				...
				searchList.promiseDeepForClass(siteRequest).onSuccess(a -> {
					try {
						if(searchList.size() >= 1) {
							FishFarm o = searchList.getList().stream().findFirst().orElse(null);
							FishFarm o2 = new FishFarm();
							o2.setSiteRequest_(siteRequest);
	...
	public Future<Void> persistFishFarm(FishFarm o, Boolean patch) {
		...
		try {
			...
			SqlConnection sqlConnection = siteRequest.getSqlConnection();
			Long pk = o.getPk();
			sqlConnection.preparedQuery("SELECT name, address, description, location, created, archived, ST_AsGeoJSON(areaServed) as areaServed, id, sessionId, userKey, ngsildTenant, ngsildPath, ngsildContext, objectTitle, ngsildData, displayPage, color FROM FishFarm WHERE pk=$1")
					.collecting(Collectors.toList())
					.execute(Tuple.of(pk)
					).onSuccess(result -> {
				try {
