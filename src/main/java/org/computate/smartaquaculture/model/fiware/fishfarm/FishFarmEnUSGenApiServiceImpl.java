...
public class FishFarmEnUSGenApiServiceImpl extends BaseApiServiceImpl implements FishFarmEnUSGenApiService {
  ...
  public Future<ServiceResponse> response200SearchPageFishFarm(SearchList<FishFarm> listFishFarm) {
    ...
    try {
      ...
      if(listFishFarm.size() == 0)
        pageTemplateUri = templateSearchPageFishFarm(siteRequest.getServiceRequest());
  ...
  public Future<Void> persistFishFarm(FishFarm o, Boolean patch) {
    ...
    try {
      ...
      SqlConnection sqlConnection = siteRequest.getSqlConnection();
      Long pk = o.getPk();
      sqlConnection.preparedQuery("SELECT name, address, description, location, created, id, entityShortId, archived, ST_AsGeoJSON(areaServed) as areaServed, ngsildTenant, ngsildPath, ngsildContext, ngsildData, sessionId, color, userKey, objectTitle, displayPage, displayPageFrFR, editPage, editPageFrFR, userPage, userPageFrFR, download, downloadFrFR FROM FishFarm WHERE pk=$1")
          .collecting(Collectors.toList())
          .execute(Tuple.of(pk)
          ).onSuccess(result -> {
        try {
