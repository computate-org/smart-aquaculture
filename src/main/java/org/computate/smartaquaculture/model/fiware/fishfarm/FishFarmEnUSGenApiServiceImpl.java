...
public class FishFarmEnUSGenApiServiceImpl extends BaseApiServiceImpl implements FishFarmEnUSGenApiService {
	...
	public void searchFishFarm(ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			if(id != null)
				form.add("permission", String.format("%s-%s#%s", FishFarm.CLASS_SIMPLE_NAME, id, "GET"));
			webClient.post(
	...
	public Future<ServiceResponse> response200SearchFishFarm(SearchList<FishFarm> listFishFarm) {
		...
		try {
			...
			if(json == null) {
				String id = siteRequest.getServiceRequest().getParams().getJsonObject("path").getString("id");
						String m = String.format("%s %s not found", "fish farm", id);
	...
	public void patchFishFarm(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			form.add("permission", String.format("%s#%s", FishFarm.CLASS_SIMPLE_NAME, "PUT"));
			if(id != null)
				form.add("permission", String.format("%s-%s#%s", FishFarm.CLASS_SIMPLE_NAME, id, "PATCH"));
			webClient.post(
					config.getInteger(ComputateConfigKeys.AUTH_PORT)
					, config.getString(ComputateConfigKeys.AUTH_HOST_NAME)
					, config.getString(ComputateConfigKeys.AUTH_TOKEN_URI)
					...
					if(authorizationDecisionResponse.failed() || !scopes.contains("PATCH")) {
						...
						searchFishFarmList(siteRequest, false, true, true).onSuccess(listFishFarm -> {
							try {
								...
								apiRequest.setId(Optional.ofNullable(listFishFarm.first()).map(o2 -> o2.getId().toString()).orElse(null));
	...
	public void postFishFarm(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			form.add("permission", String.format("%s#%s", FishFarm.CLASS_SIMPLE_NAME, "PUT"));
			if(id != null)
				form.add("permission", String.format("%s-%s#%s", FishFarm.CLASS_SIMPLE_NAME, id, "POST"));
			webClient.post(
					config.getInteger(ComputateConfigKeys.AUTH_PORT)
					, config.getString(ComputateConfigKeys.AUTH_HOST_NAME)
					, config.getString(ComputateConfigKeys.AUTH_TOKEN_URI)
	...
	public Future<FishFarm> postFishFarmFuture(SiteRequest siteRequest, Boolean id) {
		Promise<FishFarm> promise = Promise.promise();
			...
			pgPool.withTransaction(sqlConnection -> {
				...
				varsFishFarm(siteRequest).onSuccess(a -> {
					createFishFarm(siteRequest).onSuccess(fishFarm -> {
						sqlPOSTFishFarm(fishFarm, id).onSuccess(b -> {
	...
	public void deleteFishFarm(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			form.add("permission", String.format("%s#%s", FishFarm.CLASS_SIMPLE_NAME, "PUT"));
			if(id != null)
				form.add("permission", String.format("%s-%s#%s", FishFarm.CLASS_SIMPLE_NAME, id, "DELETE"));
			webClient.post(
					config.getInteger(ComputateConfigKeys.AUTH_PORT)
					, config.getString(ComputateConfigKeys.AUTH_HOST_NAME)
					, config.getString(ComputateConfigKeys.AUTH_TOKEN_URI)
	...
	public void putimportFishFarm(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			form.add("permission", String.format("%s#%s", FishFarm.CLASS_SIMPLE_NAME, "PUT"));
			if(id != null)
				form.add("permission", String.format("%s-%s#%s", FishFarm.CLASS_SIMPLE_NAME, id, "PUT"));
			webClient.post(
					config.getInteger(ComputateConfigKeys.AUTH_PORT)
					, config.getString(ComputateConfigKeys.AUTH_HOST_NAME)
					, config.getString(ComputateConfigKeys.AUTH_TOKEN_URI)
	...
	public void putimportFishFarmFuture(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			try {
				...
				apiRequest.initDeepApiRequest(siteRequest);
				siteRequest.setApiRequest_(apiRequest);
				String id = Optional.ofNullable(body.getString(FishFarm.VAR_id)).orElse(body.getString(FishFarm.VAR_solrId));
				if(Optional.ofNullable(serviceRequest.getParams()).map(p -> p.getJsonObject("query")).map( q -> q.getJsonArray("var")).orElse(new JsonArray()).stream().filter(s -> "refresh:false".equals(s)).count() > 0L) {
					siteRequest.getRequestVars().put( "refresh", "false" );
				}
				pgPool.getConnection().onSuccess(sqlConnection -> {
					String sqlQuery = String.format("select * from %s WHERE id=$1", FishFarm.CLASS_SIMPLE_NAME);
							...
							try {
								if(result.size() >= 1) {
									...
									patchFishFarmFuture(o, true).onSuccess(b -> {
										LOG.debug("Import FishFarm {} succeeded, modified FishFarm. ", body.getValue(FishFarm.VAR_id));
										eventHandler.handle(Future.succeededFuture());
									}).onFailure(ex -> {
									...
									postFishFarmFuture(siteRequest, true).onSuccess(b -> {
										LOG.debug("Import FishFarm {} succeeded, created new FishFarm. ", body.getValue(FishFarm.VAR_id));
										eventHandler.handle(Future.succeededFuture());
									}).onFailure(ex -> {
										LOG.error(String.format("putimportFishFarmFuture failed. "), ex);
										eventHandler.handle(Future.failedFuture(ex));
	...
	public void searchpageFishFarmPageInit(FishFarmPage page, SearchList<FishFarm> listFishFarm) {
	}

	public String templateSearchPageFishFarm(ServiceRequest serviceRequest) {
	...
	public Future<ServiceResponse> response200SearchPageFishFarm(SearchList<FishFarm> listFishFarm) {
		...
		try {
			SiteRequest siteRequest = listFishFarm.getSiteRequest_(SiteRequest.class);
			String pageTemplateUri = templateSearchPageFishFarm(siteRequest.getServiceRequest());
	...
	public void editpageFishFarmPageInit(FishFarmPage page, SearchList<FishFarm> listFishFarm) {
	}

	public String templateEditPageFishFarm(ServiceRequest serviceRequest) {
	...
	public Future<ServiceResponse> response200EditPageFishFarm(SearchList<FishFarm> listFishFarm) {
		...
		try {
			SiteRequest siteRequest = listFishFarm.getSiteRequest_(SiteRequest.class);
			String pageTemplateUri = templateEditPageFishFarm(siteRequest.getServiceRequest());
			String siteTemplatePath = config.getString(ComputateConfigKeys.TEMPLATE_PATH);
			Path resourceTemplatePath = Path.of(siteTemplatePath, pageTemplateUri);
			String template = siteTemplatePath == null ? Resources.toString(Resources.getResource(resourceTemplatePath.toString()), StandardCharsets.UTF_8) : Files.readString(resourceTemplatePath, Charset.forName("UTF-8"));
			FishFarmPage page = new FishFarmPage();
