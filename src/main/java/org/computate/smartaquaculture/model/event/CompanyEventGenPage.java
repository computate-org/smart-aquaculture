...
public class CompanyEventGenPage extends CompanyEventGenPageGen<PageLayout> {
   ...
   * Ignore: true
   **/
  protected void _searchListCompanyEvent_(Wrap<SearchList<CompanyEvent>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListCompanyEvent_ != null)
      w.o(Optional.ofNullable(searchListCompanyEvent_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListCompanyEvent_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListCompanyEvent_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListCompanyEvent_.getStart().longValue();
    Long rows = searchListCompanyEvent_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListCompanyEvent_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListCompanyEvent_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    CompanyEvent.varsQForClass().forEach(var -> {
      ...
      json.put("val", Optional.ofNullable(searchListCompanyEvent_.getRequest().getQuery()).filter(fq -> fq.startsWith(CompanyEvent.varIndexedCompanyEvent(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    for(String var : CompanyEvent.varsFqForClass()) {
      ...
      if(v != null) {
        ...
        while(mFq.find()) {
          String entityVar = CompanyEvent.searchVarCompanyEvent(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    CompanyEvent.varsRangeForClass().forEach(var -> {
      String varIndexed = CompanyEvent.varIndexedCompanyEvent(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(CompanyEvent.displayNameCompanyEvent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListCompanyEvent_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListCompanyEvent_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListCompanyEvent_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListCompanyEvent_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListCompanyEvent_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
    ...
    for(String sort : Optional.ofNullable(searchListCompanyEvent_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = CompanyEvent.searchVarCompanyEvent(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", CompanyEvent.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListCompanyEvent_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListCompanyEvent_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListCompanyEvent_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListCompanyEvent_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return CompanyEvent.searchVarCompanyEvent(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListCompanyEvent_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListCompanyEvent_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListCompanyEvent_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListCompanyEvent_.getFacetPivotMinCount()).orElse(0));
  }
    ...
    Optional.ofNullable(searchListCompanyEvent_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = CompanyEvent.searchVarCompanyEvent(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListCompanyEvent_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListCompanyEvent_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listCompanyEvent(JsonArray l) {
