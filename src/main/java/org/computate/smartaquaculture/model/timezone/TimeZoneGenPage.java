 ...
 * Generated: true
 **/
public class TimeZoneGenPage extends TimeZoneGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListTimeZone_(Wrap<SearchList<TimeZone>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListTimeZone_ != null)
      w.o(Optional.ofNullable(searchListTimeZone_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListTimeZone_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListTimeZone_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListTimeZone_.getStart().longValue();
    Long rows = searchListTimeZone_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListTimeZone_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListTimeZone_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    TimeZone.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(TimeZone.displayNameTimeZone(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(TimeZone.classSimpleNameTimeZone(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListTimeZone_.getRequest().getQuery()).filter(fq -> fq.startsWith(TimeZone.varIndexedTimeZone(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : TimeZone.varsFqForClass()) {
      String varIndexed = TimeZone.varIndexedTimeZone(var);
      String varStored = TimeZone.varStoredTimeZone(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(TimeZone.displayNameTimeZone(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(TimeZone.classSimpleNameTimeZone(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListTimeZone_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TimeZone.varIndexedTimeZone(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = TimeZone.searchVarTimeZone(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    TimeZone.varsRangeForClass().forEach(var -> {
      String varIndexed = TimeZone.varIndexedTimeZone(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(TimeZone.displayNameTimeZone(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(TimeZone.classSimpleNameTimeZone(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListTimeZone_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TimeZone.varIndexedTimeZone(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListTimeZone_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListTimeZone_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListTimeZone_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListTimeZone_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListTimeZone_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = TimeZone.searchVarTimeZone(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", TimeZone.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListTimeZone_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = TimeZone.searchVarTimeZone(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", TimeZone.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListTimeZone_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListTimeZone_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListTimeZone_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListTimeZone_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return TimeZone.searchVarTimeZone(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListTimeZone_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListTimeZone_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListTimeZone_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListTimeZone_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListTimeZone_.getDefaultSort()) {
      Optional.ofNullable(searchListTimeZone_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = TimeZone.searchVarTimeZone(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListTimeZone_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = TimeZone.searchVarTimeZone(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListTimeZone_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListTimeZone_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listTimeZone(JsonArray l) {
    Optional.ofNullable(searchListTimeZone_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListTimeZone_ == null ? 0 : searchListTimeZone_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<TimeZone> w) {
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("time zones");
    else if(searchListTimeZone_ == null || resultCount == 0)
      c.o("no time zone found");
    else
      c.o("time zones");
  }
  ...
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/time-zone-999.png");
  }

  @Override
  protected void _classIcon(Wrap<String> c) {
      c.o("<i class=\"fa-duotone fa-regular fa-globe\"></i>");
  }

  protected void _pageUriTimeZone(Wrap<String> c) {
